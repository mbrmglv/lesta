...
def process_documents(documents: Dict[str, str]) -> List[Dict[str, Any]]:
    ...
    doc_freq = {}
    word_doc_mapping = defaultdict(dict)  # word -> {doc_name -> count in doc}
    ...
    for word, count in word_counts.items():
        if word in doc_freq:
            ...
            doc_sources_formatted = ", ".join([f"{doc}({count})" for doc, count in doc_sources.items()])
...
def process_text(text: str, documents: List[str]) -> List[Dict[str, Any]]:
    ...
    """
    logger.info("Using legacy process_text function - converting to document dictionary")
    doc_dict = {}
