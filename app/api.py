...
from app.schemas import ErrorResponse, ResultsResponse, UploadResponse, WordInfo
...
@router.get(
    ...
    summary="Home Page",
    description="Renders the home page with file upload form",
)
    ...
    try:
        ...
        with TimingLogger(logger, "loading_documents", **logger_context):
            ...
            if not documents:
                ...
                await crud.update_text_analysis(
                    db, task_id, "failed", "No valid documents found in upload"
                )
                return
        ...
        try:
            ...
            with TimingLogger(logger, "save_results", **logger_context):
                ...
                logger.info(
                    "Word results saved to database",
                    count=len(results),
                    **logger_context,
                )
            ...
            logger.error(
                "Processing failed", error=str(e), exc_info=True, **logger_context
            )
        ...
        try:
            ...
            logger.warning(
                "Failed to delete temporary directory", error=str(e), **logger_context
            )
    ...
    try:
        ...
        with TimingLogger(logger, "file_reading", **logger_context):
            try:
                ...
                logger.warning(
                    "Failed to read with utf-8 encoding, trying cp1251",
                    **logger_context,
                )
                try:
                    with open(file_path, "r", encoding="cp1251") as f:
                        text = f.read()
                        logger.debug(
                            "File read with cp1251 encoding",
                            file_size=len(text),
                            **logger_context,
                        )
                except Exception as e:
        ...
        try:
            ...
            logger.warning(
                "Failed to delete temporary file", error=str(e), **logger_context
            )
...
async def upload_multiple_files(
    ...
    files: List[UploadFile] = File(
        ..., description="Text files to analyze (.txt or .text format)"
    ),
    ...
    request_id = id(files)
    logger.info(
        "Multiple files upload request received",
    ...
    try:
        ...
        for i, file in enumerate(files):
            ...
            if total_size > max_total_size:
                logger.warning(
                    "Upload rejected: Total files too large",
                    total_size=total_size,
                    limit=max_total_size,
                    task_id=task_id,
                )
                raise HTTPException(
                    status_code=400, detail="Total upload size too large"
                )
        ...
        logger.info(
            "Background processing task scheduled",
            task_id=task_id,
            request_id=request_id,
        )
...
@router.get(
    ...
    responses={
        200: {
            "description": "Successful response with results",
...
async def get_results(
    ...
    if analysis.status == "processing":
        logger.info(
            "Analysis still processing", task_id=task_id, status=analysis.status
        )
    ...
    is_html_request = (
        request
        and ("text/html" in accept_header or "*/*" in accept_header)
    ...
    return ResultsResponse(items=items, total=total_items, page=page, pages=total_pages)
...
async def view_results(
    ...
    if not task:
        return templates.TemplateResponse(
            "error.html", {"request": request, "message": "Task not found"}
        )
    ...
    try:
        ...
        if page > total_pages:
            return templates.TemplateResponse(
                ...
                {
                    "request": request,
                    "message": f"Invalid page number. The task has only {total_pages} page(s).",
                },
            )
